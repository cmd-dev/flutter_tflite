...
public class TflitePlugin implements MethodCallHandler {
    ...
    ByteBuffer feedInputTensorFrame(List<byte[]> bytesList, int imageHeight, int imageWidth, float mean, float std, int rotation,double hiddenRatio) throws IOException {
...
//        Log.v("image", "image height  " + String.valueOf(imageHeight));
//        Log.v("image", "image width  " + String.valueOf(imageWidth));
//        Log.v("bmp", "bitmapraw width  " + String.valueOf(bitmapRaw.getWidth()));
//        Log.v("bmp", "bitmapraw height  " + String.valueOf(bitmapRaw.getHeight()));
        bitmapRaw = Bitmap.createBitmap(bitmapRaw, 0, 0, bitmapRaw.getWidth(), bitmapRaw.getHeight(), matrix, true);
//        Log.v("bmp", "bitmapraw width before  " + String.valueOf(bitmapRaw.getWidth()));
//        Log.v("bmp", "bitmapraw height before " + String.valueOf(bitmapRaw.getHeight()));

        bitmapRaw = Bitmap.createBitmap(bitmapRaw, 0, (int) (bitmapRaw.getHeight() * hiddenRatio), bitmapRaw.getWidth(), (int) (bitmapRaw.getHeight() *(1-hiddenRatio)));
//        Log.v("bmp", "bitmapraw width  after" + String.valueOf(bitmapRaw.getWidth()));
//        Log.v("bmp", "bitmapraw height  after" + String.valueOf(bitmapRaw.getHeight()));
//
        //        Log.v("time", "Conversion took " + (SystemClock.uptimeMillis() - startTime));

//    //calculate aspect ratio
//    Bitmap croppedBitmap = null;
//    float koefX = (float) bitmapRaw.getWidth() / (float) imageWidth;
//    float koefY = (float) bitmapRaw.getHeight() / imageHeight;
//
//    //get viewfinder border size and position on the screen
//    int x1 = 0;
//    int y1 = bitmapRaw.getHeight()-200;
//
//    int x2 = bitmapRaw.getWidth();
//    int y2 = bitmapRaw.getHeight();
//
...
//    if (cropStartX + cropWidthX <= bitmapRaw.getWidth() &&
//            cropStartY + cropHeightY <= bitmapRaw.getHeight()) {
//      croppedBitmap = Bitmap.createBitmap(bitmapRaw, cropStartX,
//              cropStartY, cropWidthX, cropHeightY);
//
//      Log.v("bitmap", "bitmap took "+croppedBitmap.toString() );
//    } else {
//      croppedBitmap = bitmapRaw;
//      Log.v("bitmap", "d took "+croppedBitmap.toString() );
//    }
//
        //save result
...
//      Log.v("time", "Inference took " + (SystemClock.uptimeMillis() - startTime));
    ...
    private class RunModelOnFrame extends TfliteTask {
        ...
        RunModelOnFrame(HashMap args, Result result) throws IOException {
            ...
            imgData = feedInputTensorFrame(bytesList, imageHeight, imageWidth, IMAGE_MEAN, IMAGE_STD, rotation,1.1);
        }
...
//      Log.v("time", "Inference took " + (SystemClock.uptimeMillis() - startTime));
    ...
    void detectObjectOnFrame(HashMap args, Result result) throws IOException {
        ...
        ByteBuffer imgData = feedInputTensorFrame(bytesList, imageHeight, imageWidth, IMAGE_MEAN, IMAGE_STD, rotation,1.1);
...
//      Log.v("time", "Inference took " + (SystemClock.uptimeMillis() - startTime));
    ...
    private class RunPix2PixOnFrame extends TfliteTask {
        ...
        RunPix2PixOnFrame(HashMap args, Result result) throws IOException {
            ...
            outputType = args.get("outputType").toString();
            startTime = SystemClock.uptimeMillis();
            input = feedInputTensorFrame(bytesList, imageHeight, imageWidth, IMAGE_MEAN, IMAGE_STD, rotation,1.1);
            output = ByteBuffer.allocateDirect(input.limit());
            output.order(ByteOrder.nativeOrder());
...
//      Log.v("time", "Inference took " + (SystemClock.uptimeMillis() - startTime));
    ...
    void runPoseNetOnFrame(HashMap args, Result result) throws IOException {
        ...
        double threshold = (double) args.get("threshold");
        int nmsRadius = (int) args.get("nmsRadius");
        double hiddenRatio = (double)args.get("hiddenRatio");
        ByteBuffer imgData = feedInputTensorFrame(bytesList, imageHeight, imageWidth, IMAGE_MEAN, IMAGE_STD, rotation,hiddenRatio);
